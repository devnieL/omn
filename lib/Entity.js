...
class Entity {
  constructor(props) {
    ...
    this._id = null; // Set default values.
    ...
    if (properties && this.setProperties) this.setProperties(properties);
  }
  /** STATIC PROPERTIES */
  ...
  setProperties(data) {
    ...
    for (let propertyName in props) {
      const type = Utils.getPropertyType(self.constructor.schema, propertyName); //console.log({propertyName, type});
  ...
  toJSON(_propertiesAsObject) {
    ...
    log.debug("self.constructor.schema:", self.constructor.schema); // Properties 
  ...
  static async delete(id) {
    log.debug("Entity.delete()");
    if (typeof id === "object") throw new Error("Invalid entity id");
  ...
  static async populateProperties(instance) {
    ...
    for (let property of properties_to_populate) {
      ...
      if (Utils.isArrayProperty(properties[property])) {
        ...
        if (properties[property] != null) properties[property] = await Model.read(properties[property]);
      }
    }
    ...
    switch (this.db.type) {
      case "neo4j":
        try {
          ...
          let result = await Model.read(id); // Return null if there is no result
    ...
    switch (this.db.type) {
      case "mongodb":
        try {
          ...
          log.debug({
            instance
          }, "Entity.findOne() object to return");
          return instance;
        } catch (e) {
          log.error(e);
          throw e;
