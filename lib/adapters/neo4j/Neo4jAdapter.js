...
class Neo4jAdapter {
  static create(instance_class) {
    ...
    if (instance_class.db) {
      if (instance_class.db.type != "neo4j") throw new Error("To use a Neo4jAdapter, a Neo4j db connection should be set");
    }
  ...
  static createSchema(instance_class) {
    ...
    var parsed_schema = {}; // Parse schema for Neo4j
  ...
  async delete(param) {
    ...
    try {
      if (typeof param == "number" || typeof param == "string") await Model.delete(param);else await Model.delete(param.id);
    } catch (e) {
      throw e;
    }
  }
  ...
  async drop() {
    ...
    try {
      ...
      if (e.code === 26) log.debug("Ignored Error:", e.message);else throw e;
    }
  }
