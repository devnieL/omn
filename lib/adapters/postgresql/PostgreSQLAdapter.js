...
class PostgreSQLAdapter {
  static create(instance_class) {
    ...
    if (instance_class.db) {
      if (instance_class.db.type != "postgresql") throw new Error("To use a PostgreSQLAdapter, a PostgreSQL db connection should be set");
    }
  ...
  static createSchema(instance_class) {
    ...
    var parsed_schema = {}; // Parse schema for PostgreSQL
    ...
    for (let i in instance_class.schema) {
      ...
      if (typeof inner == "object") {
        ...
        if (inner.null != null) v.allowNull = inner.null; //value = (isArray) ? [v] : v;
  ...
  static getOrCreateModel(instance_class, schema) {
    if (typeof instance_class != "function") throw new Error("To get or create a model, a class is necessary as parameter");
  ...
  convertProperties(schema, properties) {
    ...
    for (var i in properties) {
      ...
      if (typeof properties[i] == "object") converted_properties[i] = properties[i].id;else converted_properties[i] = properties[i];
    }
  ...
  async save(entity) {
    ...
    try {
      if (entity.id) {
        ...
        let results = await Model.update(this.convertProperties(entity.constructor.schema, entity.getProperties()), {
          where: {
            id: entity.id
          }
        });
        ...
        if (results[0] != 1) throw new Error("The instance was not updated");
        return entity;
      } else {
  ...
  async query(query) {
    try {
      ...
      log.debug({
        query,
        results
      }, "PostgreSQLAdapter.query() results");
      let instances = await Promise.all(results.map(async function (result) {
        log.debug({
          result
        }, "PostgreSQLAdapter query() results map()");
        let instance = new Constructor();
        instance.id = result.id.toString();
        instance.setProperties(result);
        log.debug({
          instance
        }, "instance created from result");
        instance = await Constructor.populateProperties(instance);
        log.debug({
          instance
        }, "instance after populating properties");
        return instance;
      }));
      log.debug({
        instances
